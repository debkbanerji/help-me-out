angular.module('requestAService').component('requestAService', {
    templateUrl: 'request-a-service/request-a-service.template.html',

    controller: ['$routeParams', '$route', '$firebaseObject', '$firebaseArray', function requestAServiceController($routeParams, $route, $firebaseObject, $firebaseArray) {
        var self = this;
        self.user = firebase.auth().currentUser;
        self.userRef = firebase.database().ref().child("users").child(self.user.uid);
        self.servicesRef = firebase.database().ref().child("services");

        self.servicesList = [];
        self.servicesArray = $firebaseArray(self.servicesRef);
